...
const AiChat = () => {
	...
	const {Mentees, ...sessionUser} = useSelector(state => state.session.user);
	...
	const handleSubmit = async event => {
		...
		setMessages([...messages, {text: input, user: sessionUser}]);
		setInput('');
	...
	return (
		<div className="flex">
			<div className="bg-dark2 w-screen text-light1 flex flex-col items-center justify-center h-[750px] px-2 py-10 rounded-xl shadow-xl">
				<h1 className="mb-5 text-2xl font-bold">Chat with GPT</h1>
				...
				<div className="mt-4">
					<button
						...
						<p className="pl-2 text-sm font-medium">Clear Chat</p>
					</button>
				</div>
			</div>
		</div>
